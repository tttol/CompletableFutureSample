...
public class CompletableFutureJob {
     ...
     * @throws Exception
     */
    public void doCompletableFuture(int initialValue) throws Exception {
        log.info("[START] doAsyncTask");
        log.info("initial value = [%d]".formatted(initialValue));

        Supplier<Integer> getIntVal = () -> initialValue;
        Function<Integer, Integer> func = value -> {
            try {
                log.info("[START]重い処理");
                for (int i = 0; i < 3; i++) {
                    Thread.sleep(1000);
                    log.info("%d秒経過".formatted(i + 1));
                }
        ...
        var future = CompletableFuture.supplyAsync(getIntVal)
                .thenApply(func)
                .thenAccept(out);
